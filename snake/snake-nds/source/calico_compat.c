...
/* threadIrqWait: calico declares as u32 threadIrqWait(bool, IrqMask).
 * We use BIOS SWI 0x04 (IntrWait) instead of calico threading. */
...
/* keypadGetState: KEYINPUT (ARM9 register) + EXTKEYIN (from ARM7 shared memory).
 * EXTKEYIN (0x04000136) is ARM7-only; our ARM7 stub writes it to 0x027FF000. */
unsigned keypadGetState(void) {
    unsigned state = ~(*(volatile unsigned short *)0x04000130) & 0x03FF;
    /* ARM7 writes raw EXTKEYIN to shared memory at 0x027FF000 */
...
/* touchRead: no ARM7 SPI support, return false (no touch) */
